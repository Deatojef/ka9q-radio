...
int main(int argc,char *argv[])
{
    ...
    while(1) {
        ...
        while((size = recv(Input_fd,packet,sizeof(packet),0)) > 0) {
            ...
            // get RTP header timestamp
            time_t ts = rtp_header.timestamp;

            char result[1024];
            char result2[1024];
            printf(" %s ||||  %s ssrc %u seq %d %s\n", format_gpstime(result2, sizeof(result2), gps_time_ns() - ts), format_gpstime(result,sizeof(result),gps_time_ns()), rtp_header.ssrc, rtp_header.seq, monstring);
